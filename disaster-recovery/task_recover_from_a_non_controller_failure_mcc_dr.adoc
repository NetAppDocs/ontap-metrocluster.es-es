...
Comprueba que los LIF estén alojados en los nodos y puertos adecuados antes de realizar una conmutación de sitios.
...
CAUTION: Debes verificar que la ubicación de los LIF de datos sea correcta en los nuevos nodos antes de realizar la conmutación de sitios. Cuando realizas la conmutación de sitios de la configuración, ONTAP intenta reanudar el tráfico en el puerto principal utilizado por los LIF. Puede ocurrir un fallo de E/S cuando la conexión del puerto principal al puerto del switch y la VLAN es incorrecta. Si tienes algún problema para ubicar los LIF en los nodos o puertos correctos, no continúes con la conmutación de sitios y contacta al soporte técnico de NetApp.
...
.Si un plex falla:
--
.. Destruir el plex fallido:
+
`storage aggregate plex delete -aggregate <aggregate_name> -plex <plex_name>`

.. Restablezca la relación con el espejo recreando el espejo:
+
...
--
.Si un plex está fuera de línea:
--
En línea el plex:
...
--
.Si ambos plexos están presentes:
--
