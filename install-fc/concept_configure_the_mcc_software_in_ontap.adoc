...
---
= Configura el software MetroCluster FC en ONTAP
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Debes configurar cada nodo en la configuración de MetroCluster FC en ONTAP, incluidas las configuraciones a nivel de nodo y la configuración de los nodos en dos sitios. También debes implementar la relación de MetroCluster entre los dos sitios.

image::../media/workflow_high_level_node_and_cluster_configuration_software.gif[Flujo de trabajo para la configuración de nodos y clústeres de alto nivel]
...
Ejemplo usado en este procedimiento: site_B
 a| 
...
Ejemplo usado en este procedimiento: controller_B_1
 a| 
 a| 
 a| 
 a| 
...
Ejemplo usado en este procedimiento: controller_B_2
 a| 
 a| 
 a| 
 a| 
...
=== Configurar SNMPv3 en una configuración FC de MetroCluster

.Antes de empezar
Los protocolos de autenticación y privacidad de los switches y del sistema ONTAP deben ser los mismos.

.Acerca de esta tarea
ONTAP actualmente admite AES-128 para la privacidad (cifrado) y SHA-256 para la autenticación.

[CAUTION]
====
* En Fabric OS 10.x y versiones posteriores, Brocade solo admite SHA-512 para la autenticación SNMP. ONTAP actualmente no admite SHA-512, ya que la compatibilidad se limita a SHA-256.
* Si tus conmutadores FC MetroCluster ejecutan Fabric OS 10.x o una versión posterior, no puedes configurar SNMP ni añadir esos conmutadores al monitoreo de estado de ONTAP. Si quieres volver a una versión anterior del firmware del conmutador, puedes seguir los pasos en link:../maintain/task_upgrade_or_downgrad_the_firmware_on_a_brocade_fc_switch_mcc.html["Actualice o degrade el firmware en un switch Brocade FC"].
...
A partir de Fabric OS 9.0.1, SNMPv2 ya no es compatible con el monitoreo del estado en los switches Brocade, así que debes usar SNMPv3 en su lugar. Si estás usando SNMPv3, tienes que configurar SNMPv3 en ONTAP antes de continuar con la siguiente sección. Para más detalles, consulta <<snmpv3-mcc-fc,Configurar SNMPv3 en una configuración FC de MetroCluster>>.
