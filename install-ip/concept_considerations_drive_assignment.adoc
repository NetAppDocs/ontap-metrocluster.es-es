...
---
= Requisitos de IP de MetroCluster para la asignación automática de unidades y sistemas ADP
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
Debe tener en cuenta lo siguiente al usar ADP con configuraciones IP de MetroCluster :
...
Revise los siguientes requisitos antes de utilizar las tablas:

* Debe cumplir con los límites de MetroCluster en cuanto al número máximo de unidades admitidas y otras directrices. Consulte la https://hwu.netapp.com["Hardware Universe de NetApp"] .
* Si va a reutilizar una bandeja de discos externa, verifique que se haya eliminado la propiedad del disco en la bandeja de discos externa antes de conectarla al controlador. Referirse alink:https://docs.netapp.com/us-en/ontap/disks-aggregates/remove-ownership-disk-task.html["Eliminar la propiedad de ONTAP de un disco"^] .
...
*Nota:* La mitad de las unidades internas permanecen sin asignar antes de configurar MetroCluster.
 a| 
...
=== ADP y asignación de discos en sistemas AFF A250, AFF C250, ASA A250, ASA C250, FAS500f, AFF A20, AFF A30 y AFF C30
...
.2+| Unidades mínimas recomendadas (por sitio)  a| 
48 unidades (solo unidades externas, no unidades internas)
 a| 
Las unidades de cada bandeja externa se dividen en dos grupos iguales (mitades). Cada media bandeja se asigna automáticamente a un pool aparte.
 a| 
El par de alta disponibilidad local usa una bandeja. La segunda bandeja la utiliza el par de alta disponibilidad remoto.
...
48 unidades (unidades externas e internas)
 a| 
Las particiones internas se dividen en cuatro grupos iguales (cuartos). Cada trimestre se asigna automáticamente a un pool independiente. Las unidades en las bandejas externas se dividen en cuatro grupos iguales (cuartos). Cada bandeja de trimestres se asigna automáticamente a un pool aparte.
 a| 
...
=== ADP y asignación de discos en sistemas AFF A50 y AFF C60
...
Las unidades de cada bandeja externa se dividen en dos grupos iguales (mitades). Cada media bandeja se asigna automáticamente a un pool aparte.
 a| 
El par de alta disponibilidad local usa una bandeja. El par de alta disponibilidad remoto usa la segunda bandeja.
...
*Nota:* El agregado raíz se crea con 12 particiones raíz en el estante interno.
...
=== ADP y asignación de discos en sistemas AFF A70, AFF A90 y AFF C80
...
=== Asignación de discos en sistemas FAS9000, FAS9500, FAS70 y FAS90
...
=== Asignación de discos en los sistemas FAS50
...
=== Asignación de discos en sistemas FAS50 con caché flash

A partir de ONTAP 9.18.1, Flash Cache es compatible con los sistemas FAS50 para configuraciones IP de MetroCluster .

[CAUTION]
====
* No se pueden tener agregados de datos y el agregado raíz con unidades Flash Cache en el estante interno.
* Las ranuras 0 y 23 se utilizan para unidades de caché flash.
* Si va a reutilizar una bandeja de discos externa, verifique que se haya eliminado la propiedad del disco en la bandeja de discos externa antes de conectarla al controlador. Referirse alink:https://docs.netapp.com/us-en/ontap/disks-aggregates/remove-ownership-disk-task.html["Eliminar la propiedad de ONTAP de un disco"] .
...
No aplicable
 ...
 a| 
36 unidades (12 unidades internas y 24 unidades externas; los agregados de datos se encuentran en el estante externo y el agregado raíz en el estante interno).
 a| 
Las unidades internas se dividen en cuatro grupos iguales (cuartos).  Cada trimestre se asigna automáticamente a un grupo separado.  Las unidades en los estantes externos están divididas en cuatro grupos iguales (cuartos).  Cada cuarto de estante se asigna automáticamente a un grupo separado.

*Notas:*

* Si utiliza unidades internas y externas, primero debe instalar ONTAP y configurar el clúster local solo con las unidades internas.  Luego, conecta manualmente el estante externo una vez que hayas completado la tarea de instalación o configuración.
* Se requiere un mínimo de 12 unidades internas para el agregado raíz.  Debes colocar las unidades internas raíz desde la ranura 1.  Por ejemplo, para 12 unidades internas, utilice las ranuras 1-3, 6-8, 12-14 y 18-20.
