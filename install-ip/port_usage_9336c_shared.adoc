...
---
= Asignaciones de puertos de plataforma para switches Cisco 9336C-FX2 de 36 puertos que conectan el almacenamiento NS224 en una configuración IP de MetroCluster
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
+
Si tienes un switch Cisco 9336C-FX2 de 36 puertos y no planeas conectar almacenamiento NS224 al switch, usa las tablas en link:port_usage_3232c_9336c.html["Asignaciones de puertos de plataforma para conmutadores Cisco 3232C o Cisco 9336C-FX2 de 36 puertos"].
...
* Cuando conectas un switch Cisco 9336C-FX2 a almacenamiento NS224, solo puedes tener un máximo de dos configuraciones MetroCluster o grupos DR. Al menos una configuración MetroCluster o grupo DR debe conectar estanterías NS224 al switch MetroCluster.
+
Si una de tus configuraciones de MetroCluster o grupos DR no conecta los estantes NS224 al switch MetroCluster, sigue la <<tables_not_connecting_ns224,Tablas de cableado para controladores que no se conectan a los estantes NS224 conectados al interruptor>>.

* RcfFileGenerator solo muestra plataformas elegibles cuando se selecciona la primera plataforma.
* Si estás ampliando una configuración normal de MetroCluster que no admite almacenamiento NS224 conectado a conmutador y quieres añadir una configuración de MetroCluster que tenga almacenamiento NS224 conectado a conmutador, revisa la siguiente información:
+
** En la herramienta link:https://mysupport.netapp.com/site/tools/tool-eula/rcffilegenerator["RcfFileGenerator"^], primero tienes que seleccionar la plataforma que admite almacenamiento NS224 conectado a conmutador y luego seleccionar la plataforma que no admite almacenamiento NS224 conectado a conmutador.
+
Esto significa que cuando generes el RCF para tu nueva configuración, puede que tengas que seleccionar las plataformas en la herramienta RcfFileGenerator en el siguiente orden:

+
... Selecciona la plataforma que admite el almacenamiento NS224 conectado a conmutador en el *segundo campo desplegable*.
... Selecciona la plataforma que no admite el almacenamiento NS224 conectado a conmutador en el *primer campo desplegable*.
