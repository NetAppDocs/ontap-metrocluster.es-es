...
* Esta tarea puede realizarse en la interfaz ONTAP de cualquier nodo en la configuración IP de MetroCluster.
* A partir de ONTAP 9.18.1 y ONTAP Mediator 1.11, una única instancia de ONTAP Mediator puede gestionar hasta diez configuraciones de MetroCluster simultáneamente. En versiones anteriores, ONTAP Mediator puede admitir hasta cinco configuraciones de MetroCluster simultáneamente.
...
.Usa IPv4
--
.. Ejecuta el siguiente comando para agregar el ONTAP Mediator:
+
`metrocluster configuration-settings mediator add -mediator-address <mediator_host_ip_address>`

+

NOTE: Se te pedirá el nombre de usuario y la contraseña de la cuenta de usuario admin del Mediator.
...
. Opcionalmente, configura la conmutación forzada automática de MetroCluster.
+
Sólo puedes usar el siguiente comando en el nivel de privilegio avanzado.

+

IMPORTANT: Antes de usar este comando, revisa la link:concept-ontap-mediator-supports-automatic-unplanned-switchover.html#mauso-9-12-1["Riesgos y limitaciones del uso de conmutación forzada automática de MetroCluster"].
