...
* A partir de ONTAP 9.18.1 y ONTAP Mediator 1.11, una sola instancia de ONTAP Mediator puede administrar hasta diez configuraciones de MetroCluster simultáneamente.  En versiones anteriores, ONTAP Mediator puede admitir hasta cinco configuraciones de MetroCluster simultáneamente.
...
.Utilice IPv4
--
.. Ejecute el siguiente comando para agregar el mediador ONTAP :
+
`metrocluster configuration-settings mediator add -mediator-address <mediator_host_ip_address>`

+

NOTE: Se le solicitará el nombre de usuario y la contraseña de la cuenta de administrador de Mediator.
