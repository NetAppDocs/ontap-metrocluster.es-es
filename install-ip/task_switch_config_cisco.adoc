...
== Restablezca el conmutador IP de Cisco a los valores predeterminados de fábrica
...
* Si estás siguiendo estos pasos como parte de otro procedimiento, como la sustitución o actualización de un conmutador, comprueba si necesitas trasladar las LIF del clúster al conmutador asociado para garantizar el funcionamiento ininterrumpido del clúster. link:task_install_and_cable_the_mcc_components.html["Conoce cómo configurar los switches IP MetroCluster"].
...
== Descargue e instale el software Cisco Switch NX-OS
...
.Acerca de esta tarea
Esta tarea requiere un software de transferencia de archivos, como FTP, TFTP, SFTP, HTTP o SCP, para copiar los archivos a los switches.

* Si estás siguiendo estos pasos como parte de otro procedimiento, como la sustitución o actualización de un conmutador, comprueba si necesitas trasladar las LIF del clúster al conmutador asociado para garantizar el funcionamiento ininterrumpido del clúster. link:task_install_and_cable_the_mcc_components.html["Conoce cómo configurar los switches IP MetroCluster"].
* Debes utilizar la versión de software del conmutador compatible. Consulta https://hwu.netapp.com["Hardware Universe de NetApp"].
...
. Repite los pasos anteriores en los tres switches IP restantes de la configuración IP de MetroCluster.
+

NOTE: Debes actualizar los conmutadores en el siguiente orden: Switch_A_1, Switch_B_1, Switch_A_2, Switch_B_2. Si tu procedimiento requiere que muevas las LIF del clúster, debes repetir los pasos para mover las LIF del clúster en la segunda estructura (Switch_A_2, Switch_B_2) después de configurar los conmutadores en la primera estructura (Switch_A_1, Switch_B_1). Consulta link:task_install_and_cable_the_mcc_components.html["Conoce cómo configurar los switches IP MetroCluster"].

. Comprueba que la reversión automática esté activada para las LIF del clúster:
+
`network interface show -vserver Cluster -fields auto-revert`

+

CAUTION: Debes asegurarte de que la reversión automática esté habilitada para las LIF del clúster.

. Si la función de reversión automática no está activada, modifica todas las LIF del clúster para activar la reversión automática:
+

NOTE: Si estás siguiendo estos pasos como parte de otro procedimiento, como la sustitución o actualización de un conmutador, es posible que hayas desactivado la reversión automática como parte de los pasos para link:switch-move-cluster-lifs.html["Mueve las LIF del clúster"].
...
== Descargue e instale los archivos Cisco IP RCF
