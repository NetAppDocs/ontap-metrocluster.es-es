...
NOTE: Si estás utilizando un switch Cisco 9336C-FX2 en una configuración compartida que conecta estantes NS224, cambiar los ISL podría requerir un nuevo archivo de configuración de referencia (RCF). No necesitas un nuevo RCF si las velocidades actual y nueva de los ISL son 40 Gbps y 100 Gbps. Cualquier otro cambio en la velocidad de los ISL requiere un nuevo RCF. Por ejemplo, cambiar la velocidad de los ISL de 40 Gbps a 100 Gbps no requiere un nuevo RCF, pero cambiar la velocidad de los ISL de 10 Gbps a 40 Gbps sí requiere un nuevo RCF.
...
NOTE: Si estás utilizando conmutadores Cisco 9336C-FX2 en una configuración compartida que conecta módulos NS224 y necesitas actualizar el RCF y aplicar la nueva configuración para los nuevos puertos ISL, sigue los pasos para link:task_upgrade_rcf_files_on_mcc_ip_switches.html["Actualiza los RCF en los switches IP de MetroCluster."]
