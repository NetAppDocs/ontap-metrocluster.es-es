...
. Apague el puente ATTO y retire los cables de alimentación conectados al puente.
...
.. Si va a configurar un FibreBridge 7500N, configure el modo de conexión que el puerto utiliza para 'ptp'.
+
...
. Conecte los cables FC a los mismos puertos del nuevo puente.
. Actualice el firmware de FiberBridge en cada puente.
+
Si el nuevo puente es del mismo tipo que el puente asociado, actualice al mismo firmware que el puente asociado. Si el nuevo puente es de un tipo diferente al puente asociado, actualice al firmware más reciente admitido por el puente y la versión de ONTAP. Consulte link:task_update_firmware_on_a_fibrebridge_bridge_parent_topic.html["Actualización del firmware en un puente FiberBridge"]

. [[step18-Reconect-newbridge]] Vuelva a conectar los cables SAS a los mismos puertos del nuevo puente.
+
Es necesario sustituir los cables que conectan el puente a la parte superior o inferior de la pila de la bandeja. Los puentes FibreBridge 7600N y 7500N requieren cables mini-SAS para estas conexiones.
...
. [[step19-verify-each-bridge]]Verifique que cada puente pueda ver todas las unidades de disco y las bandejas de discos a las que está conectado el puente.
+
[cols="35,65"]
|===
...
Correcto
 a| 
Repita <<step19-verify-each-bridge,Paso 19>> para cada puente restante.
...
No es correcto
 a| 
.. Compruebe si hay cables SAS sueltos o corrija el cableado SAS repitiendo <<step18-reconnect-newbridge,Paso 18>>.
.. Repetir <<step19-verify-each-bridge,Paso 19>>.
...
. [[step18-reconnect-cables]]Vuelva a conectar los cables SAS a los mismos puertos del puente nuevo.
+
...
No es correcto
 a| 
.. Compruebe si hay cables SAS sueltos o corrija el cableado SAS repitiendo <<step18-reconnect-cables,Paso 18>>.
...
*** El puente FibreBridge 7600N admite hasta 32, 16 o 8 Gbps.
*** El puente FibreBridge 7500N admite hasta 16, 8 o 4 Gbps.
*** El puente FibreBridge 6500N admite hasta 8, 4 o 2 Gbps.
+
...
. Conecte los cables FC a los puertos Fibre Channel 1 en el nuevo puente.
+
Debe conectar mediante cable el puerto FC al mismo switch o puerto del controlador al que se había conectado el puente FibreBridge 6500N.

. Actualice el firmware de FiberBridge en cada puente.
+
Si el nuevo puente es del mismo tipo que el puente asociado, actualice al mismo firmware que el puente asociado. Si el nuevo puente es de un tipo diferente al puente asociado, actualice al firmware y la versión más recientes de ONTAP compatibles con el puente.

+
link:task_update_firmware_on_a_fibrebridge_bridge_parent_topic.html["Actualización del firmware en un puente FiberBridge"]

. [[step18-recable]]Vuelva a conectar los cables SAS a los puertos SAS A en el nuevo puente.
+
...
No es correcto
 a| 
.. Compruebe si hay cables SAS sueltos o corrija el cableado SAS repitiendo <<step18-recable,Paso 18>>.
.. Repita el paso anterior para cada puente restante.
...
. Una vez sustituida la pieza, devuelve la pieza defectuosa a NetApp, como se describe en las instrucciones de RMA incluidas con el kit. Consulta la página link:https://mysupport.netapp.com/site/info/rma["Retorno de artículo  sustituciones"^] para obtener más información.
