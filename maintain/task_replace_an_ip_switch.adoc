...
CAUTION: Si necesitas sustituir un conmutador en las plataformas AFF A220, AFF A250 o FAS2750, consulta link:https://kb.netapp.com/on-prem/ontap/mc/MC-KBs/Cluster_and_HA_communication_issues_during_back-end_switch_replacement["el artículo de base de conocimientos «Cluster and HA communication issues during back-end switch replacement»"^].
...
[NOTE]
====
Las velocidades de enlace mixtas no son compatibles con las plataformas que utilizan una tarjeta de interfaz de red X60134A. Para comprobar si tu plataforma se ve afectada, consulta el  https://hwu.netapp.com/["Hardware Universe"^].

Si vas a sustituir un conmutador que utiliza puertos de 25G por uno nuevo que utilice puertos de 100G en una plataforma afectada (por ejemplo, si sustituyes un conmutador BES-53248 por un conmutador Cisco 9336C en una plataforma AFF A50), consulta link:https://kb.netapp.com/on-prem/ontap/mc/MC-KBs/100G_link_not_coming_online_during_MetroCluster_back-end_switch_replacement?caseId=2010523490["el artículo de base de conocimientos "100G link not coming online during MetroCluster back-end switch replacement""^].
...
NOTE: Debes actualizar los conmutadores en el siguiente orden: Switch_A_1, Switch_B_1, Switch_A_2, Switch_B_2. Debes repetir los pasos para mover las LIF del clúster en la segunda estructura (Switch_A_2, Switch_B_2) después de haber configurado los conmutadores en la primera estructura (Switch_A_1, Switch_B_1). Consulta link:../install-ip/task_install_and_cable_the_mcc_components.html["Conoce cómo configurar los switches IP MetroCluster"].
...
+
Puedes seguir los pasos en link:../install-ip/using_rcf_generator.html["Conecte los cables de los switches IP de MetroCluster"].
...
+
... Usa el procedimiento link:https://docs.netapp.com/us-en/ontap-systems-switches/switch-bes-53248/migrate-to-2n-switched.html["Migrar a un entorno de clúster de NetApp conmutado"^] para convertir un clúster sin switches en un clúster conmutado. Usa los puertos indicados en link:https://docs.netapp.com/us-en/ontap-metrocluster/install-ip/using_rcf_generator.html["Instalación y configuración de IP de MetroCluster"] o los archivos de cableado RCF para conectar la interfaz local del clúster.
...
+
Si el nuevo conmutador es idéntico, enciéndelo. Si vas a actualizar o revertir la versión de los conmutadores, enciende ambos. La configuración puede funcionar con dos conmutadores diferentes en cada emplazamiento hasta que se actualice la segunda estructura.

. Repita para comprobar que la configuración de MetroCluster sea correcta <<all_step1,Paso 1>>.
+
Si vas a actualizar o degradar los switches en la primera estructura, es posible que veas algunas alertas relacionadas con la agrupación en clústeres locales.

+

NOTE: Si actualizas o vuelves a una versión anterior de la fabric, repite todos los pasos para la segunda fabric.
