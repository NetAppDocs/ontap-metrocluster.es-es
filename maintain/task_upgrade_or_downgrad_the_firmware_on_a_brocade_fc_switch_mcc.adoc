...
---
= Actualiza o revierte el firmware en un switch FC Brocade MetroCluster
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
.Antes de empezar
Lee atentamente la siguiente información antes de usar este procedimiento:

* A partir de Fabric OS 10.x, Brocade solo admite SHA-512 para la autenticación SNMP. ONTAP actualmente no admite SHA-512, ya que su compatibilidad se limita a SHA-256.
+
Si actualizas el firmware de tu conmutador FC MetroCluster a Fabric OS 10.x, se perderá la visibilidad de la supervisión SNMP desde ONTAP. Esto conlleva un mayor riesgo al solucionar problemas y la pérdida de alertas proactivas.

+
Si actualizas a o vuelves a una versión anterior de Fabric OS 10.x, debes realizar las tareas obligatorias descritas en link:https://techdocs.broadcom.com/us/en/fibre-channel-networking/fabric-os/fabric-os-software-upgrade/10-0-x/Upgrading-and-Downgrading-Firmware/v24512876.html["Guía de actualización del software Brocade Fabric OS, 10.0.x - requisitos previos para la actualización o la degradación"^].

+

CAUTION: Debes tener muy en cuenta estas limitaciones antes de proceder a actualizar a o volver a una versión anterior de Fabric OS 10.x o versiones posteriores.

* A partir de Fabric OS 9.0.1, SNMPv2 no es compatible en los switches Brocade. Si actualizas a Fabric OS 9.0.1 o una versión posterior, debes usar SNMPv3 para el monitoreo del estado. Para más información, consulta link:../install-fc/concept_configure_the_mcc_software_in_ontap.html#snmpv3-mcc-fc["Configuración de SNMPv3 en una configuración MetroCluster"].
* Si vas a actualizar a Fabric OS 9.2.x o una versión posterior, debes tener instalado un certificado Brocade TruFOS. Consulta link:https://techdocs.broadcom.com/us/en/fibre-channel-networking/fabric-os/fabric-os-software-upgrade/9-2-x/Obtaining-Firmware/download-prerequisites-new/brocade-trufos-certificates.html["Guía de actualización del software de sistema operativo Brocade Fabric, 9,2.x"^] para obtener más información.
