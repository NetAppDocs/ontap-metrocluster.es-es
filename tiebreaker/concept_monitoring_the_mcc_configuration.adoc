...
== Desactivar el modo de observador de desempate

Puede desactivar el modo de observador para que el software Tiebreaker inicie automáticamente un cambio después de detectar una falla en el sitio.


NOTE: En Tiebreaker 1.6 y versiones posteriores, puede habilitar el cambio automático cuando agrega un monitor mediante el `monitor add wizard` dominio. Referirse alink:concept_configuring_the_tiebreaker_software.html#add-mcc-config-tb["Agregue configuraciones de MetroCluster"] .

.Paso
. Deshabilitar el modo observador:
+
