...
summary: El software tiebreaker 1,4 proporciona funcionalidades de supervisión para un entorno de almacenamiento en clúster. También envía notificaciones SNMP en el caso de problemas de conectividad de los nodos y de desastres del sitio. 
---
= Instale las dependencias de MetroCluster tiebreaker 1,4
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
Según el sistema operativo Linux del host, instale un servidor MySQL o MariaDB antes de instalar o actualizar el software tiebreaker.

.Pasos
. <<install-java-1-8,Instale JDK>>.
...
Debe instalar JDK en el sistema host antes de instalar o actualizar el software tiebreaker. Tiebreaker 1,4 y versiones anteriores son compatibles con JDK 1,8.0. (JRE 8).
...
----
+
Cuando el inicio de sesión de MySQL funciona según lo esperado, la salida termina en el `mysql>` prompt.
...
.Pasos
. Inicie sesión como `root` usuario.
+
[listing]
----
...
.El futuro
link:install-tiebreaker-package-14.html["Instale el paquete de software"]
