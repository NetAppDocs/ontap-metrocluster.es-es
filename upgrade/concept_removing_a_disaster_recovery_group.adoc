...
NOTE: Utilizas estos pasos durante los flujos de trabajo de transición y actualización del sistema. Es posible que ya hayas completado algunos pasos de este procedimiento como parte de estos flujos de trabajo.
...
NOTE: La `metrocluster remove-dr-group` El comando solo se admite en ONTAP 9.8 y versiones posteriores.
...
+
En una configuración de MetroCluster FC, ve al paso para <<fc_step_7_remove_nodes,eliminando los nodos antiguos del grupo DR>>.
...
+
Se muestran todos los discos, incluidos los discos Pool1 propiedad del nodo. El siguiente ejemplo muestra la salida truncada donde el disco 0n.4 y sus particiones se asignan a Pool1. La salida variará dependiendo de tu configuración.
...
+
En el siguiente ejemplo, la propiedad de los discos particionados se elimina antes de eliminar la propiedad del disco contenedor `0n.4`:
...
. [[fc_step_7_remove_nodes]]Elimina los nodos del antiguo grupo DR.
+
...
Más información sobre los comandos utilizados en este procedimiento en el link:https://docs.netapp.com/us-en/ontap-cli/["Referencia de comandos de ONTAP"^].
