...
Las siguientes tablas muestran las combinaciones de plataformas compatibles para las configuraciones IP de ocho nodos de MetroCluster.
...
image:../media/expand-ip-group-1-updated.png["Combinaciones de expansión para plataformas del grupo 1 de AFF y FAS en MetroCluster IP"]
...
image:../media/expand-ip-group-2-updated.png["Combinaciones de expansión para plataformas del grupo 2 de AFF y FAS en MetroCluster IP"]
...
== Eliminar la configuración de las aplicaciones de supervisión
...
== Prepara los nuevos módulos controladores
...
. Desde el modo de mantenimiento, emite el comando halt para salir del modo de mantenimiento:
+
`halt`

. Comprueba si el partner-sysid es correcto:
+
...
. Arranca el sistema y ve a la configuración del clúster:
+
`boot_ontap`
...
== Configura LIFs intercluster para cluster peering
...
+
Para conocer la sintaxis completa de los comandos, consulta la link:https://docs.netapp.com/us-en/ontap-cli["Referencia de comandos de ONTAP"^].
...
+
El siguiente ejemplo asigna los puertos "e0e" y "e0f" al grupo de conmutación por error "intercluster01" en SVM "cluster01":
...
+
El siguiente ejemplo muestra que los LIF intercluster "cluster01_icl01" y "cluster01_icl02" en el puerto "e0e" harán conmutación por error al puerto "e0f".
...
+
Deberías ver una salida como la del siguiente ejemplo:
...
== Crea las interfaces MetroCluster y los agregados raíz reflejados

Debes crear las interfaces MetroCluster en los nuevos nodos IP MetroCluster y reflejar los agregados raíz.
...
.Pasos
. En cada sitio, agrega las nuevas LIF de intercluster a la relación entre iguales.
+
Desde cluster_A, ejecuta el siguiente comando:
...
+
Desde cluster_B, ejecuta el siguiente comando:
...
NOTE: La salida del comando  `metrocluster configuration-settings connection show` puede variar dependiendo del modelo de tu plataforma. Por ejemplo, en algunas plataformas, las líneas para HA Partner no se muestran en la salida.
...
== Finaliza la adición de los nuevos nodos
