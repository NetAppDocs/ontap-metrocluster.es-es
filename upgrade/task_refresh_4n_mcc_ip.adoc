...
* La tabla muestra todos los modelos de plataforma que usan *puertos compartidos de MetroCluster/HA* y *puertos compartidos de clúster/HA*. No todas las combinaciones son compatibles con este procedimiento. Consulta la link:concept_choosing_tech_refresh_mcc.html#supported-metrocluster-ip-tech-refresh-combinations["combinaciones de plataformas compatibles"] para verificar si tu combinación de actualización es compatible.
...
Tiebreaker
 a| 
link:../tiebreaker/concept_configuring_the_tiebreaker_software.html#add-mcc-config-tb["Adición de configuraciones de MetroCluster"] en _Instalación y configuración de MetroCluster Tiebreaker_.
