...
+
El estado HA de todos los componentes debe ser  `mcc`.
...
. En cada nodo, comprueba la fecha, la hora y la zona horaria del sistema:
+
`show date`

. Si es necesario, configura la fecha en UTC o en hora media de Greenwich (GMT):
+
`set date <mm/dd/yyyy>`

. Comprueba la hora utilizando el siguiente comando en el indicador del entorno de arranque:
+
`show time`

. Si es necesario, configura la hora en UTC o GMT:
+
`set time <hh:mm:ss>`
...
|===
. Comprueba el modo de ha-config:
+
`ha-config show`
...
. Siga los pasos que se indican a continuación: link:../tiebreaker/concept_configuring_the_tiebreaker_software.html#add-mcc-config-tb["Adición de configuraciones de MetroCluster"] en la sección _Instalación y configuración de MetroCluster Tiebreaker_.
