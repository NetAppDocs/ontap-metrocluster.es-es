...
.Antes de empezar
* Verifica que puedes acceder a un servidor HTTP con el sistema.
...
.Pasos
. Prepara los conmutadores IP para la aplicación de los nuevos archivos de configuración de referencia (RCF).
+
Siga los pasos de la sección para su proveedor de switches:
...
NOTE: Cuando ejecutas la opción del menú de arranque `6`, es posible que tengas que restablecer los valores de los bootarg de MetroCluster. Consulta link:task_upgrade_A700_to_A900_in_a_four_node_mcc_ip_us_switchover_and_switchback.html#set-the-metrocluster-ip-bootarg-variables["Establezca las variables bootarg de MetroCluster IP"].
...
Tiebreaker
 a| 
link:../tiebreaker/concept_configuring_the_tiebreaker_software.html#add-mcc-config-tb["Adición de configuraciones de MetroCluster"] en la sección _Instalación y configuración de MetroCluster Tiebreaker_.
