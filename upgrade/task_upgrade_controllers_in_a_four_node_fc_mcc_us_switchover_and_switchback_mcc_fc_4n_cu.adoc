...
. Si los nodos MetroCluster están configurados con un gestor de claves externo para el SVM administrador (clúster), copia la información del servidor de claves en una ubicación segura. Si el gestor de claves integrado está configurado, copia la información de copia de seguridad y la frase de contraseña en una ubicación segura.
+
[role="tabbed-block"]
====
.Gestor de llaves a bordo
--
.. Ejecuta el siguiente comando para mostrar la copia de seguridad de Onboard Key Manager:
+
`security key-manager onboard show-backup`

+
También necesitarás la frase de contraseña más actualizada de Onboard Key Manager más adelante en el procedimiento de actualización.
...
--
Reúne la información externa de gestión de claves que necesitarás para restaurar la configuración del gestor de claves más adelante en el procedimiento.

.. Muestra el nombre del SVM administrador (cluster).
+
`vserver show -type admin`

+
En este ejemplo, el nombre de la SVM admin es cluster_B:
...
cluster_B
            admin   -          -          -           -          -
----
.. Obtén la dirección y el número de puerto de un servidor de claves primario asociado con el gestor de claves externo configurado en el SVM administrador (cluster):
+
[NOTE]
====
*** La SVM de administración puede tener hasta cuatro servidores de claves primarios, pero solo necesitarás el número de puerto y la dirección de uno de los servidores de claves primarios para restaurar la configuración de gestión de claves más adelante en el procedimiento.
*** Si hay un nombre de usuario y una contraseña asociados al servidor de claves primarias que elijas, también necesitarás esa información para el proceso de recuperación.
...
----
.. Si estás ejecutando ONTAP 9.16.1 o posterior, obtén el UUID del almacén de claves para el SVM admin.
+
...
cluster_B::*
----
.. Guarda la información que recopilaste en un lugar seguro para usarla más adelante en el procedimiento. Ahora deberías haber recopilado la siguiente información:
+
*** El número de puerto y la dirección de uno de los servidores de claves primarios para el SVM admin
*** El nombre de usuario y la contraseña si están asociados al servidor de claves principal elegido
...
....
. Ingresar `n` para omitir la recuperación de la copia de seguridad cuando vea un mensaje similar al siguiente:
+
....
...
. Si se utiliza el cifrado de volumen raíz, dependiendo de la versión de ONTAP que estás usando, selecciona la opción del menú de arranque o ejecuta de nuevo el comando del menú de arranque para tu configuración de gestión de claves.
+
[role="tabbed-block"]
====
.ONTAP 9,8 y versiones posteriores
