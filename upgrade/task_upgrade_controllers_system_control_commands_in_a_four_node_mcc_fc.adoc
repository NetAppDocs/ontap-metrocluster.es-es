...
.Acerca de esta tarea
La operación de automatización inicia la conmutación, `heal-aggregates`, y. `heal root-aggregates` operaciones. Una vez completadas estas operaciones, la operación se detiene en *pausa para la intervención del usuario*, de modo que pueda montar en rack e instalar los controladores, arrancar los controladores asociados y reasignar los discos de agregado raíz al nuevo módulo de controlador desde la copia de seguridad flash, utilizando el `sysids` reunidos antes.
