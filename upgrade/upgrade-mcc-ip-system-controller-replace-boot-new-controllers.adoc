...
+
Usa los pasos en link:upgrade-mcc-ip-system-controller-replace-apply-rcf-set-bootarg.html#set-the-metrocluster-ip-bootarg-variables["Establezca las variables bootarg de MetroCluster IP"].

. Verifica que el partner-sysid es correcto:
+
`printenv partner-sysid`
...
NOTE: Cuando ejecutas la opción del menú de arranque `6`, es posible que tengas que restablecer los valores de bootarg de MetroCluster. Consulta link:upgrade-mcc-ip-system-controller-replace-apply-rcf-set-bootarg.html#set-the-metrocluster-ip-bootarg-variables["Establezca las variables bootarg de MetroCluster IP"].
...
.. Después de que los LIF interclúster estén activados, verifica el estado del clúster de iguales y restablece la interconexión de clústeres según sea necesario.
+
